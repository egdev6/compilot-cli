...
export const addComponentActions = (
	actions: any,
	data: DataProps,
	srcRoot: string,
	config: any,
	...
	if (data?.type === "component") {
		const folderName = getFolderByConfig(
			config.components.naming.folder,
			data.name || "",
		);
		const basePath = path.join(
			srcRoot,
			`${config.components.base}/{{folder}}/${folderName}/`,
		);
		const language = config.config.language;
		const templatesPath = path.join(
			srcRoot,
			`source/plop-files/templates/${language}/component`,
		);
		...
		if (data?.folder) {
			...
			const groupName = config.components.atomic
