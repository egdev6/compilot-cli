...
export const addPageActions = (actions: any, data: any, srcRoot: string, config: any) => {
  if (data?.type === 'page') {
    const folderName = kebabCase(data.name);
    const folderPath =
      kebabCase(data.folder) === folderName ? `/${kebabCase(data.folder)}` : `/${formatPath(data.folder, kebabCase)}/`;
